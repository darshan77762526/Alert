...
# Function to parse string cookies into a format Playwright can inject
...
async def scan_shein_playwright(keyword, price_limit):
    ...
    async with async_playwright() as p:
        ...
        # Inject your live cookies into the browser context
        await context.add_cookies(parse_cookies(SHEIN_COOKIE_STRING))
        page = await context.new_page()
        
        try:
            # Go to page and explicitly wait for the product grid to render
            await page.goto(url, wait_until="networkidle", timeout=30000)
            
            # Extract HTML after JavaScript has populated the products
            html = await page.content()
            links = re.findall(r'href=[\'"](/[^\'"]+-p-\d+\.html)[\'"]', html)
            unique_links = list(set(links))
            print(f"Playwright successfully rendered {len(unique_links)} products!")
