...
def scan_shein_web(keyword, price_limit):
    print(f"Scanning live website for: {keyword} under ₹{price_limit}...")
    
    # Format the keyword for the URL search string
    query = keyword.replace(" ", "%20")
    
    # Direct website scraping to bypass the broken API
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Extract product links directly from the webpage HTML
            links = re.findall(r'href=[\'"](/[^\'"]+-p-\d+\.html)[\'"]', html)
            unique_links = list(set(links))
            print(f"Found {len(unique_links)} cheap products on the page!")
            
            # Send top 5 cheapest to avoid flooding your Telegram
            for link in unique_links[:5]:
                product_id = link.split("-p-")[-1].replace(".html", "")
                full_url = f"https://www.shein.com{link}"
            ...
            print(f"Website rejected request with code: {response.status_code}")
            
    except Exception as e:
        print(f"Scanning anomaly caught: {e}")

if __name__ == "__main__":
    # Test run with ₹1000 limit. (Change these to "250" after you confirm it works!)
    scan_shein_web("men t-shirt", "1000")
    time.sleep(3)
    scan_shein_web("men pants", "1000")
